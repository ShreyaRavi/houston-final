...
public class Teleop {
	...
	public void update() {
		...
		if (true) {
			Shooter.getInstance().update(JoystickIO.btnShooter.isDown(), JoystickIO.btnAgitator.isDown(), true);
		} else {
			...
			if (JoystickIO.btnHoldLeft.isDown()) {
				...
				IO.drive.drive(0, 0, SmartDashboard.getNumber("Min Rotate", Constants.MIN_ROTATE_SPEED_POS), 0);
